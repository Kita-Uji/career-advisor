# Requirements: Career Advisor UI Polish

**Defined:** 2026-03-03
**Core Value:** The chat interface feels calm, focused, and trustworthy — users stay in conversation, not distracted by the UI.

## v1 Requirements

### Message Bubbles

- [ ] **BUBBLE-01**: Assistant messages display without the CA avatar circle
- [ ] **BUBBLE-02**: Assistant messages use a soft indigo-50 background instead of white
- [ ] **BUBBLE-03**: User messages remain visually unchanged (indigo-600 bubble)

### Input Area

- [ ] **INPUT-01**: Input container displays as an elevated floating card (shadow-lg, rounded-2xl)
- [ ] **INPUT-02**: Input card lifts off the bottom bar visually (padding/spacing adjusted to feel floating)

## v2 Requirements

*(none — scope is intentionally narrow)*

## Out of Scope

| Feature | Reason |
|---------|--------|
| Header / branding changes | Not requested |
| Color scheme overhaul | User happy with indigo palette |
| New features (history, export) | Not in scope for this polish pass |
| Layout restructuring | Only MessageBubble and ChatInput touch points |

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| BUBBLE-01 | Phase 1 | Pending |
| BUBBLE-02 | Phase 1 | Pending |
| BUBBLE-03 | Phase 1 | Pending |
| INPUT-01 | Phase 1 | Pending |
| INPUT-02 | Phase 1 | Pending |

**Coverage:**
- v1 requirements: 5 total
- Mapped to phases: 5
- Unmapped: 0 ✓

---
*Requirements defined: 2026-03-03*
*Last updated: 2026-03-03 after roadmap creation*
